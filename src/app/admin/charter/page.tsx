...
const initialForm: Form = {
  ...
  sunset_drink: "", champagne_supplement: false,
...
export default function AdminCharterPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const sunsetOptions = normalizeManualCharterSunsetOptions(
        form.formule,
        form.sunset_drink,
        form.champagne_supplement
      );
      const response = await fetch("/api/admin/charter", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, ...sunsetOptions }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error);
