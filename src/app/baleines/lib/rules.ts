...
export const AGE_MINIMUM_MISE_EAU = 12;
...
export function afficherMateriel(participant: Participant) {
  return (
    ...
    ageAutoriseMiseEau(participant.age) &&
    participant.materielPerso === false
  );
}

export function ageAutoriseMiseEau(ageValue: string) {
  const age = Number(ageValue);
  return Number.isFinite(age) && age >= AGE_MINIMUM_MISE_EAU;
}

export function ageRenseigneMineur(ageValue: string) {
  const age = Number(ageValue);
